<div>
    <x-alert />
    <x-breadcrumbs defaultSegment="Produk" />
        ...
        <div
            ...
            <div class="flex flex-col items-center justify-between p-4 space-y-3 md:flex-row md:space-y-0 md:space-x-4">
                ...
                <div
                    ...
                    @can('menambah produk')
                        <a type="button" href="{{ route('products.create') }}" wire:navigate
                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            ...
                            <th scope="col" class="px-4 py-3">Harga Dasar</th>
                    ...
                    <tbody>
                        @forelse ($products as $index => $product)
                            <tr class="border-b dark:border-gray-700">
                                ...
                                <td class="px-4 py-3">Rp. {{ $product->initial_price }}</td>
                                <td class="px-4 py-3">{{ "$product->percentage_profit%" }}</td>
                                <td class="px-4 py-3">Rp. {{ $product->sale_price }}</td>
                                    ...
                                    @canany(['mengubah produk', 'menghapus produk'])
                                        <div class="dropdown dropdown-left dropdown-end">
                                            ...
                                            <ul tabindex="0"
                                                ...
                                                @can('mengubah produk')
                                                    <li>
                                                        <a href="{{ route('products.edit', $product->name) }}" wire:navigate
                                                            class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Edit</a>
                                                    </li>
                                                @endcan
                                                @can('menghapus produk')
                                                    <li>
                                                        <!-- Open the modal using ID.showModal() method -->
                                                        <a onclick="my_modal_1.showModal()"
                                                            wire:click="getDataForDelete({{ $product }})"
